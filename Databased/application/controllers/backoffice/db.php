...
class Db extends CI_Controller {
	...
	*/
	...
	public function index($db, $table = '', $clearSearch = false) 
	{
		...
		$this->load->view('backoffice/table/table', $this->data);
	...
	*/
	...
	public function newTable($db)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->data['success_message'] = sprintf( $this->lang->line('db_newtable_success'), site_url('backoffice/db/'.$db.'/'.$_POST['tableName']) );							
	...
	*/
	...
	public function deleteTable($db, $table)
	{
		...
		if( $this->tablemodel->getReferencedTables($db, $table) != false ) {
			...
			$temp['error_message'] .= "</ul><br>".sprintf( $this->lang->line('db_deletetable_error3_part2'), site_url('backoffice/db/'.$db.'/'.$table), $table );
	...
	*/
	...
	{		
		...
		if (1 == FALSE) {
			...
			$this->data['success_message'] = sprintf( $this->lang->line('db_uploadcsv_success'), site_url('backoffice/db/'.$db.'/'.$_POST['tableName']) );				
	...
	public function importCsv($db)
	{
		...
		if ( ! $this->upload->do_upload('thefile')) {
			...
			if( $res ==  true ) {
				...
				$this->data['success_message'] = sprintf( $this->lang->line('db_importcsv_success'), site_url('backoffice/db/'.$db.'/'.$_POST['tableName']) );
