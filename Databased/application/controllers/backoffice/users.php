...
class Users extends CI_Controller {
	...
	*/
	...
	public function index($userID = "")
	{
		...
		$this->load->view('backoffice/users/users', $this->data);
	...
	*/
	...
	public function create() 
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			redirect("backoffice/users/", "refresh");
			...
			$this->email->message( $this->load->view('backoffice/auth/email/new', array('email'=>$_POST['email'], 'password'=>$_POST['password']), true ) );	
			
			$this->email->send();
			
			//echo $this->email->print_debugger();
			
			redirect("backoffice/users/".$userID, "refresh");
	...
	*/
	...
	public function updateLogin($userID)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->session->set_flashdata('success_message', $this->lang->line('users_updatelogin_success'));
	...
	*/
	...
	public function delete($userID)
	{
		...
		if( $this->usermodel->adminUsers() && $userID != 1 ) {
			...
			$this->session->set_flashdata('success_message', $this->lang->line('users_delete_success'));
