...
class Columns extends CI_Controller {
	...
	*/
	...
	public function getDetails($db = '', $table = '', $column = '')
	{
		...
		$return['column'] = $this->load->view('backoffice/partials/column_edit', $columnStuff, true);
		
		$columnRestrictions = $this->tablemodel->getColumnRestrictions($db, $table, $column);
				
		$return['columnRestrictions'] = $this->load->view('backoffice/partials/columnrestrictions', array('columnRestrictions'=>$columnRestrictions), true);
		...
		$return['notes'] = $this->load->view("backoffice/partials/columnnotes", array('notes'=>$columnNotes), true);
	...
	*/
	...
	public function update($db = '', $table = '')
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$return['columns'] = $this->load->view("backoffice/partials/column_table", $cData, true);
	...
	*/
	...
	public function addColumn($db = '', $table = '')
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->data['success_message'] = sprintf( $this->lang->line('colums_addcolumn_success'), site_url('backoffice/db/'.$db.'/'.$table) );
