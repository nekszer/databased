...
class Crud extends CI_Controller {
	...
	*/
	...
	public function index($db, $table = '', $clearSearch = false) 
	{
		...
		if( $clearSearch == true ) {
			...
			redirect(site_url("backoffice/db/".$db."/".$table), 'location');
		...
		if( isset( $_POST['columns'] ) && $_POST['db'] == $db && $_POST['table'] == $table ) {
			...
			redirect(site_url("backoffice/crud/".$db."/".$table), 'location');
		}
		...
		$this->load->view('backoffice/crud/table', $this->data);
	...
	*/
	...
	public function saveField($db = '', $table = '', $column = '') 
	{
		...
		if($db == '' || $db == 'undefined' || $table == '' || $table == 'undefined' || $column == '' || $column == 'undefined') {
			...
			$return['message'] = $this->load->view('backoffice/partials/message_error', $this->data, true);
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$return['revisions'] = $this->load->view("backoffice/partials/cellrevisions", array('revisions'=>$revisions), true);
				
			$return['response_code'] = 1;
			$return['success_message'] = $this->load->view("backoffice/partials/message_success", $this->data, true);
	...
	*/
	...
	public function getRecord($db = '', $table = '', $indexName = '', $recordID = '', $param = 'editrecord') 
	{
		...
		if( $param == 'editrecord' ) {
			...
			$return['record'] = $this->load->view('backoffice/partials/record', $recordArray, true);
		
		} elseif( $param == 'viewrecord' ) {
		
			$recordArray['mode'] = 'view';
			$return['record'] = $this->load->view('backoffice/partials/viewrecord', $recordArray, true);
	...
	*/
	...
	public function updateRecord($db = '', $table = '', $indexName = '', $recordID = '')
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$return['response_code'] = 1;
			$return['message'] = $this->load->view('backoffice/partials/message_success', $this->data, true);
			...
			$return['revisions'] = $this->load->view("backoffice/partials/revisions", $revisionsData, true);
	...
	*/
	...
	public function newRecord($db, $table)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$return['newrecordform'] = $this->load->view('backoffice/partials/newrecordform', array('tableFields_'=>$allFields, 'theDB'=>$db, 'theTable'=>$table), true);
	...
	*/
	...
	public function deleteRecord($db, $table, $recordID)
	{
		...
		redirect('backoffice/crud/'.$db."/".$table, "refresh");
	...
	*/
	...
	public function newTable($db)
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->data['success_message'] = sprintf( $this->lang->line('db_newtable_success'), site_url('backoffice/crud/'.$db.'/'.$_POST['tableName']) );							
	...
	*/
	...
	public function deleteTable($db, $table)
	{
		...
		if( $this->tablemodel->getReferencedTables($db, $table) != false ) {
			...
			$temp['error_message'] .= "</ul><br>".sprintf( $this->lang->line('db_deletetable_error3_part2'), site_url('backoffice/crud/'.$db.'/'.$table), $table );
		...
		redirect('backoffice/db/'.$db, "refresh");
	...
	*/
	...
	public function getCell($db = '', $table = '', $column = '', $recordID = '')
	{
		...
		$return['cell'] = $this->load->view('backoffice/partials/cell', array('cell'=>$cell), true);
	...
	*/
	...
	public function updateTable($db = '', $table = '')
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			redirect('backoffice/db/'.$db."/".$table, 'refresh');
			...
			redirect('backoffice/db/'.$db."/".$_POST['tableName'], 'refresh');
	...
	{		
		...
		if (1 == FALSE) {
			...
			if ( ! $this->upload->do_upload('thefile')) {
				...
				$suggestedDilimiter = $this->csvreader->guessDelimiter(base_url('backoffice')."/tmp/".$fileData['file_name']);
				...
				if( isset($_POST['columns']) && $_POST['columns'] == 'yes' ) {
					...
					$file = fopen(base_url('backoffice')."/tmp/".$fileData['file_name'],"r");
			...
			$this->data['success_message'] = sprintf( $this->lang->line('db_uploadcsv_success'), site_url('backoffice/crud/'.$db.'/'.$_POST['tableName']) );				
	...
	public function importCsv($db)
	{
		...
		if ( ! $this->upload->do_upload('thefile')) {
			...
			if( $res ==  true ) {
				...
				$this->data['success_message'] = sprintf( $this->lang->line('db_importcsv_success'), site_url('backoffice/crud/'.$db.'/'.$_POST['tableName']) );
